  ...
  const fetchProduct = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setImages(new Array(product.image.length).fill(null)); // maintain the same length for uploaded files
      } else {
    ...
    <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-3">
      ...
      <div>
        ...
        <div className="flex gap-2">
          {(existingImages.length > 0 || images.length > 0 ? Array.from({ length: Math.max(existingImages.length, images.length) }) : []).map((_, idx) => {
              ...
              <label key={idx} className="cursor-pointer">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (f) handleImageChange(idx, f);
                  }}
                />
              </label>
            );
          })}
          {/* Show one extra slot if total images < 4 */}
